...
const CONFIG = {
  ...
  typing: {
    phrases: [
      'The homebrew store for jailbroken 360s...',
      'Games. Emulators. Utilities. Themes.',
      'RGH | JTAG | BadUpdate ready.',
      'Paid Beta soon → Free after.',
...
/* ═══════════════════════════════════════════════════════════
   3. SCROLL REVEAL — fade/slide in on scroll
   ═══════════════════════════════════════════════════════════ */
(function initReveal() {
...
/* ═══════════════════════════════════════════════════════════
   4. GITHUB STATS — live fetch from GitHub API
   ═══════════════════════════════════════════════════════════ */
(async function initGitHubStats() {
  ...
  // Skip if not on a page with stats
  ...
  try {
    ...
    // Fallback: show dashes if API fails (rate limit, offline, etc.)
...
/* ═══════════════════════════════════════════════════════════
   7. PARALLAX GLOWS — subtle background movement
   ═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   8. HERO LOGO TILT — subtle 3D tilt on mouse move
   ═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   9. TYPING CURSOR BLINK — restart animation on phrase change
   ═══════════════════════════════════════════════════════════ */
(function initCursorBlink() {
  const cursor = document.querySelector('.cursor');
  if (!cursor) return;

  // Cursor already blinks via CSS — this just keeps it in sync
  // if the browser throttles animations on inactive tabs.
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
      cursor.style.animation = 'none';
      void cursor.offsetHeight; // reflow
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
(function easterEgg() {
  const style = 'color:#FF6B00;font-weight:800;font-size:14px;';
  const green = 'color:#107C10;font-weight:800;font-size:14px;';

  console.log('%c🔓 JAIL X360', style);
  console.log('%cBreak free. Play more.', green);
  console.log('%cMade with ❤️ by Frosty-Dev-Xbox', style);
  console.log('%chttps://github.com/Frosty-Dev-Xbox/Jail-X360', green);
})();
